import {Project29_backend } from "../../declarations/Project29_backend";
...
appointForm.addEventListener("submit", async (e) => {
  ...
  try {
    const getId = await Project29_backend.addTodo(obj.textarea);
...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    const getAll = awaitProject29_backend.getAllTodo();
...
async function deleteTodo(userId) {
  try {
    ...
    awaitProject29_backend.deleteTodo(userId);
